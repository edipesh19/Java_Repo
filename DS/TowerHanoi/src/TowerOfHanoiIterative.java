...
public class TowerOfHanoiIterative {
	...
	public void execute() {
		...
		for (int i = 0; i < limit; i++) {
			...
			int ff = movements(i, disk);
			int gg = direction(disk, n);
			int source = (ff * gg) % 3; // SOURCE PEG
			int destination = (source + gg) % 3; // DESTINATION
																	// PEG
	...
	private int movements(int i, int d) { // HOW MANY TIMES DISK d HAS MOVED
											// BEFORE STAGE i
		while (d-- != 0)
			i /= 2;
		return (i + 1) / 2;
	...
	private int direction(int d, int n) { // EACH DISK MOVES IN SAME DIRECTION
											// CW=1, ACW=2
		return 2 - (n + d) % 2;
	}
